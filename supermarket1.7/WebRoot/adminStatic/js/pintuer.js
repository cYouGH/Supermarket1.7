$(function() {
	...
	$('textarea, input, select').blur(function() {
		...
		if (e.attr("data-validate")) {
			...
			if ($checkvalue != ""
				...
				for (var i = 0; i < $checkdata.length; i++) {
					...
					if (!$pintuercheck(e, $checktype[0], $checkvalue)) {
						$checkstate = false;
						$checktext = $checktext + "<li>" + $checktype[1]
								+ "</li>";
					}
				}
			};
			if ($checkstate) {
				...
				e.closest('.field').append('<div class="input-help"><ul>'
						+ $checktext + '</ul></div>');
			}
		}
	});
	$pintuercheck = function(element, type, value) {
		...
		switch (type) {
			...
			case "zip" :
				...
				break;
			case "radio" :
				var radio = element.closest('form').find('input[name="'
						+ element.attr("name") + '"]:checked').length;
				return eval(radio == 1);
				break;
			default :
				var $test = type.split('#');
					...
					switch ($test[0]) {
						...
						case "length" :
							...
							if (element.attr("type") == "checkbox") {
								$length = element.closest('form')
										.find('input[name="'
												+ element.attr("name")
												+ '"]:checked').length;
							} else {
								$length = $pintu.replace(/[\u4e00-\u9fa5]/g,
										"***").length;
							}
	...
	$('.form-reset').click(function() {
		...
		$(this).closest('form').find('.form-group')
	...
	$('.banner').each(function() {
		...
		if (pointer != 0 && page > 1) {
			...
			e.append(pager);
			pager.css("left", e.outerWidth() * 0.5 - pager.outerWidth() * 0.5
							+ "px");
	...
	$(".fixed").each(function() {
		...
		$(window).bind("scroll", function() {
					...
					var thisbottom = top - $(window).scrollTop()
							- $(window).height();
